	...
	return &corev1.Service{
		...
		Spec: corev1.ServiceSpec{
			...
			Ports: []corev1.ServicePort{
				{Name: ldapPortName, Port: r.ldapPort(ds), TargetPort: intstr.FromString(ldapPortName)},
				{Name: ldapsPortName, Port: r.ldapsPort(ds), TargetPort: intstr.FromString(ldapsPortName)},
			},
...
func (r *DirectoryServiceReconciler) desiredStatefulSet(
	...
	container := corev1.Container{
		...
		Ports: []corev1.ContainerPort{
			{Name: ldapPortName, ContainerPort: ldapPort, Protocol: corev1.ProtocolTCP},
			{Name: ldapsPortName, ContainerPort: ldapsPort, Protocol: corev1.ProtocolTCP},
		},
		...
		ReadinessProbe: &corev1.Probe{
			ProbeHandler: corev1.ProbeHandler{
				TCPSocket: &corev1.TCPSocketAction{
					Port: intstr.FromString(ldapPortName),
				},
			},
...
func (r *DirectoryServiceReconciler) recordEvent(
	...
	if r.Recorder != nil {
		r.Recorder.Eventf(ds, nil, eventType, reason, "%s", message)
	}
}
